...
{
    public class BaseODBC : IDatabaseConnection
    {
        ...
        public BaseODBC(string mode, string connectionString)
        {
            this._mode = mode;
            ...
            //if (!connectionString.Trim().Equals("") && connectionString.Length > 10)
            {
                try
                {
                    this._conn = new OdbcConnection(connectionString);
                }
                catch (Exception ex) { Console.WriteLine(ex.ToString());}
            }
            this._ds = new DataSet();
            //this.eventLogWriter = new EventLogWriter("BaseODBC", EventLogWriter.EventViewerLogType.Application);
        }
        public BaseODBC(string mode, string connectionString, string eventLogWriterSourceID)
        {
            this._mode = mode;
            this._connectionString = connectionString;
            this._errString = "";
        ...
        public bool getData(OdbcCommand cmd, bool closeConnection, int timeOut)
        {
            if (this._conn.State == ConnectionState.Closed)
            {
                if (!OpenConnection())
        ...
        public DataTable getDataTable(OdbcCommand cmd, bool closeConnection, int timeOut)
        {
            if (this._conn.State == ConnectionState.Closed)
            {
                if (!OpenConnection())
        ...
        /// <returns></returns>
        public OdbcDataReader getDataToReader(string sql, bool closeConnection)
        {
            if (this._conn.State == ConnectionState.Closed)
            {
                if (!OpenConnection())
            ...
            this._command = new OdbcCommand(sql, this._conn);
            OdbcDataReader rdr = this._command.ExecuteReader();

            if (closeConnection)
                CloseConnection();
            return rdr;
        }
        public OdbcDataReader getDataToReader(OdbcCommand cmd, bool closeConnection)
        {
            if (this._conn.State == ConnectionState.Closed)
            {
                if (!OpenConnection())
        ...
        public int executeSQL(OdbcCommand cmd, bool closeConnection)
        {
            if (!OpenConnection())
                return -1;
        ...
        public IDataReader executeSP(string SpName, DbParameterCollection parms)
        {
            ...
            this._command = new OdbcCommand(SpName, this._conn);
            this._command.CommandType = CommandType.StoredProcedure;
            if (parms != null)
                for (int i = 0; i < parms.Count; ++i)
                {
                    OdbcParameter p = new OdbcParameter(parms[i].ParameterName, parms[i].Value);
        ...
        public DataSet executeSPDataSet(string SpName, DbParameterCollection parms)
        {
            ...
            OdbcDataAdapter adp = new OdbcDataAdapter(this._command);
            int j = 0;
            try
            {
                if ((j = adp.Fill(ds)) < 0)
        ...
        #endregion public DataSet executeSPDataSet(string SpName, DbParameterCollection parms, int timeout)
        ...
        public string getSingleValue(OdbcCommand cmd)
        {
            this._isErr = false;
            this._errString = "";
        ...
        private bool FillDataSet(string sql, int timeOut)
        {
            ...
            try
            {
                ...
                if (timeOut == 0)
                    ...
                    this._command.CommandTimeout = timeOut;
                OdbcDataAdapter sqlAdapter = new OdbcDataAdapter();
        ...
        private bool FillDataSet(OdbcCommand cmd, int timeOut)
        {
            this._errString = "";
            this._isErr = false;
            if (this._conn == null)
        ...
        private int executeSQLLocal(string sql)
        {
            ...
            OdbcTransaction trans = this._conn.BeginTransaction(IsolationLevel.ReadCommitted);
        ...
        private int executeSQLLocal(OdbcCommand cmd)
        {
            int x = 0;
