...
{
    public class BaseSQL : IDatabaseConnection
    {
        ...
        public BaseSQL(string mode, string connectionString)
        {
            this._mode = mode;
            ...
            //this.eventLogWriter = new EventLogWriter("BaseSQL", EventLogWriter.EventViewerLogType.Application);
        }
        public BaseSQL(string mode, string connectionString, string eventLogWriterSourceID)
        {
            this._mode = mode;
            this._connectionString = connectionString;
            this._errString = "";
        ...
        public bool getData(SqlCommand cmd, bool closeConnection, int timeOut)
        {
            if (this._conn.State == ConnectionState.Closed)
            {
                if (!OpenConnection())
        ...
        public DataTable getDataTable(SqlCommand cmd, bool closeConnection, int timeout)
        {
            if (this._conn.State == ConnectionState.Closed)
            {
                if (!OpenConnection())
        ...
        public SqlDataReader getDataToReader(string sql, bool closeConnection)
        {
            ...
            this._command = new SqlCommand(sql, this._conn);
            SqlDataReader rdr = this._command.ExecuteReader();

            if (closeConnection)
                CloseConnection();
            return rdr;
        }
        public SqlDataReader getDataToReader(SqlCommand cmd, bool closeConnection)
        {
            if (this._conn.State == ConnectionState.Closed)
            {
                if (!OpenConnection())
        ...
        public int executeSQL(SqlCommand cmd, bool closeConnection)
        {
            if (!OpenConnection())
                return -1;
        ...
        public IDataReader executeSP(string SpName, DbParameterCollection parms)
        {
            ...
            this._command = new SqlCommand(SpName, this._conn);
            this._command.CommandType = CommandType.StoredProcedure;
            if (parms != null)
                for (int i = 0; i < parms.Count; ++i)
                {
                    SqlParameter p = new SqlParameter(parms[i].ParameterName, parms[i].Value);
        ...
        #region public IDataReader executeSP(string SpName, DbParameterCollection parms, int timeout)
        /// <summary>
        /// executes a stored procedure and maintains all commits and errors
        /// </summary>
        /// <param name="SpName">stored procedure name</param>
        ...
        #endregion public IDataReader executeSP(string SpName, DbParameterCollection parms, int timeout)
        ...
        public DataSet executeSPDataSet(string SpName, DbParameterCollection parms)
        {
            ...
            SqlDataAdapter adp = new SqlDataAdapter(this._command);
            int j = 0;
            try
            {
                if ((j = adp.Fill(ds)) < 0)
        ...
        public string getSingleValue(SqlCommand cmd)
        {
            this._isErr = false;
            this._errString = "";
        ...
        public bool FillDataSet(SqlCommand cmd, int timeOut)
        {
            this._errString = "";
            this._isErr = false;
            if (this._conn == null)
        ...
        private int executeSQLLocal(string sql)
        {
            ...
            SqlTransaction trans = this._conn.BeginTransaction("Transaction");
        ...
        private int executeSQLLocal(SqlCommand cmd)
        {
            int x = 0;
